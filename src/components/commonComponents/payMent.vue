  ...
  export default {
    ...
    methods: {
      ...
      async getZFBPay(){
        ...
        let res = await pay(this.payUrl, this.orderId, channel , this.couponId,  baseURL.URL, spbillCreateIp);
        if(res.error_code == 2000){
          localStorage.setItem('H5_isChoosed',false);
          localStorage.setItem('H5_cancel_counp_state',true);
          if(res.data.isZero == 1){
            Toast({
                message:'已为您成功预约车位',
                duration:1500
      ...
      goCounp(){
        ...
        localStorage.setItem('counpFlag',false);
        this.$router.push({
          name:'counp'
        })
      },
    ...
    created(){
      ...
      this.timeAdd();
      // 定义一个事件，做一个定时器 (当前的时间-保存的时间 > 5 分钟的话，那么就要提示用户了)
      // document.addEventListener("visibilitychange", vm.documentEvent)function(){
      //   if(document.visibilityState=='visible'){
      //      // 获取 当前的时间 和 保存的时间  miunte 表示时间
      //     clearInterval(vm.timeInter);
      //     let timeStamp = Date.parse(new Date());
      ...
      document.addEventListener("visibilitychange", vm.documentEvent);
       ...
       this.discounts = this.$route.params.amount;
       console.log(this.payFees);
    ...
    activated () {
      ...
      this.isChoosed = JSON.parse(localStorage.getItem('H5_isChoosed'));
      if(!this.isChoosed){
        this.discounts = 0;
        this.couponId = -1;
      }
