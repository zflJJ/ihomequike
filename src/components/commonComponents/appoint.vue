    ...
    <div class="content p-a" ref="apponitBox">
      <div>
        ...
        <div class="tips">
          ...
          <div class="tips-text">3.超过5分钟未进场订单自动取消。</div>
...
import {appointCarport,pay} from '../../server/getData';
...
//          },
  ...
  methods: {
      ...
      if(res.error_code === 2000){
        ...
        if(!this.reserveTimeList.length){
          ...
          for(let i=0;i<this.reserveTimeList.length;i++){
            // this.filterTime(this.reserveTimeList[i])
            console.log(this.reserveTimeList[i].startTime);
            console.log(formatTimeStamp(this.reserveTimeList[i].startTime),formatTimeStamp(this.reserveTimeList[i].endTime));
          }
    ...
    convertTime(){
      ...
      // 取得默认入场时间-----这里有问题
      this.defaultTime = hoursarray[0].text +':' +  pData2[hoursarray[0].value][0].text;
      // 通过时间戳差值，获取价格信息
    ...
    confirmFn(e){
      ...
      // debugger
      this.getPrice(this.priceTime);
      this.defaultTime = e.select1.text+':'+e.select2.text;
    ...
    async goApoint(){
      ...
      this.params.user_id = JSON.parse(localStorage.getItem('H5_user_id'));;
