  ...
  export default {
    ...
    methods: {
      ...
      chooseItems(item,index){
        console.log(item,index,item.state,item.isChosen);
...
//        }
        ...
        if(item.isChosen){ //取消优惠券
          ...
          localStorage.setItem('H5_discounts',0);
          // localStorage.removeItem('H5_counpinfo');
...
//      console.log(counpinfo);
//      if(counpinfo !== null){
//        this.currentIndex =counpinfo.couponId;
//      }
      this.getCounpLists();
    },
    activated () {
      //这里不用每次刷新是为了保存再次进入取消的状态
      this.orderId = localStorage.getItem('H5_orderId');
      var counpFlag = localStorage.getItem('counpFlag');
      if(counpFlag == "true"){
        this.pageNum = 0;
        this.getCounpLists();
        this.currentIndex = null;
      }else {
        let coupStateCancel = JSON.parse(localStorage.getItem('H5_cancel_counp_state'));
        if(coupStateCancel){
          this.currentIndex = null;
        }
      }
