    ...
    <div class="appoit-info-box" ref="appoitInfoBox">
      <div>
        <div class="content">
          ...
          <div class="guide-show">
            ...
            <div class="guide-info">
              <span class="name-text">
                ...
                <template v-if="orderData.parklocNumber !== null">
                  {{orderData.parklocNumber}}
                </template>
                <!--<span v-show="orderData.parklocNumber !== null">{{orderData.parklocNumber}}</span>-->
              </span>
              <span class="price-text"><span>￥</span>&nbsp;{{orderData.totalFee}}</span>
            </div>
          ...
          <!-- 预约支付明细 -->
          <template v-if="orderData.payChannelReserve !== null">
          ...
          <template v-if="orderData.state === 1303 || orderData.state === 1304">
            ...
            <div class="dingdan-info">
              ...
              <div class="gm-flex gm-height" v-if="orderData.overTimeFee !== null">
...
import {canCelMyAppoint,queryMyactive,getOrderInfo} from '../../server/getData';
  ...
  methods: {
    ...
    dispoceOrderDat(objDatas){
      ...
      if(objDatas.parkingCouponFee === null){
        ...
        let payFee = null;
        payFee = objDatas.parkingFee - objDatas.reserveCouponFee;
      ...
      // if(objDatas.state === 1300){
      //   objDatas.dstateInfo = '未支付'
      // }else if(objDatas.state === 1301 || objDatas.state === 1302){
      //   objDatas.dstateInfo = '进行中'
      // }else if(objDatas.state === 1304){
      //   objDatas.dstateInfo = '已支付'
      // }else if(objDatas.state === 1309){
      //   objDatas.dstateInfo = '已取消'
      // }else if(objDatas.state === 1308 || objDatas.state === 1307 || objDatas.state === 1310){
      //   objDatas.dstateInfo = '超时已取消'
      // }
...
//      if(res.errmsg == 'success'){
//        //刷新预约车位列表页面
//        // this.$root.eventHub.$emit('refresh-appoiment-list');
//        Toast('已成功取消预约');
//          this.$router.go(-1);
//      }else{
//        Toast('取消预约失败');
