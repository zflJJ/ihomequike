  ...
  <div id="suggestions">
    ...
    <div class="content-box" ref="suggestionBox">
      <div class="content">
        ...
        <div class="feed-back">
          <div class="title">备注(120字)</div>
          <div class="text-box p-r">
            <div class="text">
              <textarea name="feedBack" id="feedBack" cols="30" rows="4" placeholder="至少10个字" v-model="textVal" @input="limitSubs"></textarea>
            </div>
            <span class="cls-text p-a" @click="clearText" v-show="textVal.trim().length"></span>
            <div class="words-show p-a"><span>{{wordsNum}}</span><span>/120</span></div>
          </div>
        ...
        <div class="comfirm t-c" ><span class="t-c" @click="submitSuggestions">提交</span></div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async submitSuggestions(){
      ...
      this.$root.eventHub.$http.post(URL+'apiwrite/feedback/add',formData,config).then((res)=>{
        console.log(res);
...
#suggestions
  ...
  .choice-item
    padding .75rem 0 .75rem 1.5625rem
    color #1a1a1a
    font-size 1rem
    font-weight normal
    // text-align center
